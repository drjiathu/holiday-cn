...
def get_paper(url: str) -> str:
    ...
    assert re.match(
        r"http://www.gov.cn/zhengce/content/\d{4}-\d{2}/\d{2}/content_\d+.htm",
        url,
...
class DescriptionParser:
    ...
    def get_date(self, year: int | None, month: int | None, day: int) -> date:
        ...
        if (year is None and month == 12 and self.date_history and
                max(self.date_history) < date(year=self.year, month=2, day=1)):
            year = self.year - 1
...
class SentenceParser:
    ...
    def _extract_dates_1(self, value: str) -> Iterator[date]:
        ...
        for groups in match:
            ...
            yield self.parent.get_date(groups[0], groups[1], groups[2])

    def _extract_dates_2(self, value: str) -> Iterator[date]:
        value = re.sub(r"（[^>]+）", "", value)
        match = re.findall(
            r"(?:(\d+)年)?(?:(\d+)月)?(\d+)日(?:[至\-—])?(?:(\d+)年)?(?:(\d+)月)?(\d+)日",  # noqa
            value
        )
        for groups in match:
            groups = [_cast_int(i) for i in groups]
...
def _raise_for_status_200(resp: requests.Response):
    ...
    if resp.status_code != 200:
        raise requests.HTTPError(
            f"request failed: {resp.status_code}: {resp.request.url}",
            response=resp,
        )
